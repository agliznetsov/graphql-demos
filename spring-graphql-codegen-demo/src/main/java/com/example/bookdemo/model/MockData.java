...
public abstract class MockData {
	...
	static {
		...
		AUTHORS.add(Author.newBuilder().id(1).firstName("Joanne").lastName("Rowling").build());
		AUTHORS.add(Author.newBuilder().id(2).firstName("Herman").lastName("Melville").build());

		BOOKS = new ArrayList<>();
		BOOKS.add(Book.newBuilder().id(1).name("Harry Potter and the Philosopher's Stone").pageCount(223).authorId(1)
				.build());
		BOOKS.add(Book.newBuilder().id(2).name("Moby Dick").pageCount(635).authorId(2).build());

		int bookId = 3;
		for (int i = 3; i <= 100; i++) {
			AUTHORS.add(Author.newBuilder().id(i).firstName(randomName()).lastName(randomName()).build());
			for (int n = 0; n < 5; n++) {
				BOOKS.add(Book.newBuilder().id(bookId).name(randomName()).pageCount(randomInt())
						.authorId(i)
						.build());
				bookId++;
			}
	...
	public static Map<Book,List<Comment>> getBookComments(List<Book> books) {
		return books.stream()
				.collect(Collectors.toMap(it -> it, it -> Arrays.asList(
						Comment.newBuilder().text("comment #1 for book " + it.getId()).build(),
						Comment.newBuilder().text("comment #2 for book " + it.getId()).build(),
						Comment.newBuilder().text("comment #3 for book " + it.getId()).build()
				)));
	}
